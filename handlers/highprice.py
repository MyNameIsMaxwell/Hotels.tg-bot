...
from states.states import MyStatesHighprice


@bot.message_handler(commands=['highprice'])
@logger.catch()
def highprice_menu(message: Message) -> None:
	"""
	Функция, реагирующая на команду 'highprice' и предлагающая ввести название города.

	:param message: сообщение Telegram
	"""
	bot.send_message(message.chat.id, "Давай узнаем топ самых дорогих отелей в городе!\nКакой город?")
	bot.set_state(message.from_user.id, MyStatesHighprice.city_highprice, message.chat.id)


@bot.message_handler(state=MyStatesHighprice.city_highprice, is_digit=False)
@logger.catch()
def city_get(message: Message) -> None:
	"""
	Функция, ожидающая корректный ввод города.
	Состояние пользователя - 'city_highprice', если в названии города не было цифр.
	...
	if cities:
		...
		bot.set_state(message.from_user.id, MyStatesHighprice.hotels_count_highprice, message.chat.id)
	else:
...
@bot.message_handler(state=MyStatesHighprice.hotels_count_highprice, is_digit=False)
@logger.catch()
...
@bot.message_handler(state=MyStatesHighprice.hotels_count_highprice, is_digit=True)
@logger.catch()
def hotels_count_step(message: Message) -> None:
	...
	bot.set_state(message.from_user.id, MyStatesHighprice.load_photo_highprice, message.chat.id)
	with bot.retrieve_data(message.from_user.id, message.chat.id) as data:
		data['hotels_count'] = hotels_count


@bot.message_handler(state=MyStatesHighprice.load_photo_highprice)
@logger.catch()
def load_photo_step(message: Message) -> None:
	...
	if load_photo == u'Да':
		...
		bot.set_state(message.from_user.id, MyStatesHighprice.photo_count_highprice, message.chat.id)
...
@bot.message_handler(state=MyStatesHighprice.photo_count_highprice, is_digit=True)
@logger.catch()
...
@bot.message_handler(state=MyStatesHighprice.photo_count_highprice, is_digit=False)
@logger.catch()
...
@logger.catch()
def ready_for_answer_highprice(message: Message) -> None:
	"""
