...
def lowprice_menu(message):
	bot.send_message(message.chat.id, 'Давай узнаем топ самых дешёвых отелей в городе!\nКакой город?')
	bot.set_state(message.from_user.id, MyStates.city_lowprice, message.chat.id)
...
@bot.message_handler(state=MyStates.city_lowprice)
def city_get(message):
	...
	bot.set_state(message.from_user.id, MyStates.hotels_count_lowprice, message.chat.id)
	with bot.retrieve_data(message.from_user.id, message.chat.id) as data:
		data['city'] = message.text


@bot.message_handler(state=MyStates.hotels_count_lowprice, is_digit=False)
def hotels_count_incorrect(message):
	bot.send_message(message.chat.id, 'Не похоже на число. Введите число, пожалуйста')


@bot.message_handler(state=MyStates.hotels_count_lowprice, is_digit=True)
def hotels_count_step(message):
	...
	bot.set_state(message.from_user.id, MyStates.load_photo_lowprice, message.chat.id)
	with bot.retrieve_data(message.from_user.id, message.chat.id) as data:
		data['hotels_count'] = hotels_count


@bot.message_handler(state=MyStates.load_photo_lowprice)
def load_photo_step(message):
	...
	if load_photo == u'Да':
		...
		bot.set_state(message.from_user.id, MyStates.photo_count_lowprice, message.chat.id)
	else:
		bot.send_message(message.chat.id, 'Тогда вот что мы нашли!')
		ready_for_answer(message)


@bot.message_handler(state=MyStates.photo_count_lowprice, is_digit=True)
...
@bot.message_handler(state=MyStates.photo_count_lowprice, is_digit=False)
...
def ready_for_answer(message):
	...
	if 'photo_count' not in locals():
		history.low_high_history_add(user_id, "lowprice", city_name, hotel_info)
	else:
		history.low_high_history_add(user_id, "lowprice", city_name, hotel_info, photo_count)
	bot.delete_state(message.from_user.id, message.chat.id)
